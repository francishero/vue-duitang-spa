<template lang="html">
  <div class="all">
    <div class="article-head">
        <mt-header fixed title="Club">
            <router-link to="/discovery" slot="left">
...
export default {
  mounted () {
    this.axios.get('https://www.easy-mock.com/mock/5a1d88738e6ddb24964d081b/duitang/club/recommend')
      .then((response) => {
  ...
  methods: {
    ...
    switchNav (url) {
      // console.log(url)
      this.axios.get('https://www.easy-mock.com/mock/5a1d88738e6ddb24964d081b/duitang/club/' + url)
        .then((response) => {
        // 选出选中的
        let tempclubItems = response.data.map(item => {
          let oItem = item
